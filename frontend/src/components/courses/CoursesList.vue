      ...
      >
        ...
        <div class="course-card-header" :style="{ background: getCourseGradient(course.id) }">
          <div class="flex flex-col gap-1">
            ...
            <span v-if="isEnrolled(course.id)" class="inline-flex items-center gap-1 text-xs text-white/90 font-medium">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4" /></svg>
              Записан
            </span>
          </div>
        </div>
        ...
        <div class="p-5">
          ...
          <div v-if="isEnrolled(course.id) && getProgress(course) !== null" class="mb-4">
            <div class="flex justify-between text-xs text-slate-500 mb-1">
              <span>Прогресс</span>
              <span class="font-semibold text-sky-600">{{ getProgress(course) }}%</span>
            </div>
            <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
              <div class="h-full bg-gradient-to-r from-sky-500 to-cyan-400 rounded-full transition-all duration-500"
                :style="{ width: `${getProgress(course)}%` }"></div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            <router-link 
              ...
              class="btn btn-primary text-sm py-2 flex-1 justify-center"
            >
              <svg v-if="isEnrolled(course.id)" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              ...
              {{ isEnrolled(course.id) ? 'Продолжить' : 'Подробнее' }}
            </router-link>
            <button
              v-if="!authStore.isTeacher && !isEnrolled(course.id) && course.status === 'published'"
              class="btn btn-secondary text-sm py-2 px-3"
              :disabled="enrollingId === course.id"
              @click.prevent="enroll(course)"
            >
              <svg v-if="enrollingId === course.id" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
            ...
            <span v-if="isEnrolled(course.id)" class="badge badge-success text-xs">Записан</span>
          </div>
        ...
        <div class="flex items-center gap-2 shrink-0">
          ...
          >
            {{ isEnrolled(course.id) ? 'Продолжить' : 'Открыть' }}
          </router-link>
        </div>
      </div>
    </div>
...
const isEnrolled = (courseId) => studentStore.isEnrolled(courseId)
const getProgress = (course) => {
  const enrollment = studentStore.enrollments.find(e => e.id === Number(course.id))
...
const enroll = async (course) => {
  ...
  try {
    await studentStore.enroll(course.id)
  } catch (e) {
...
onMounted(async () => {
  ...
  if (authStore.isAuthenticated && !authStore.isTeacher) {
    studentStore.fetchEnrollments().catch(() => {})
  }
  loading.value = false
})
</script>
